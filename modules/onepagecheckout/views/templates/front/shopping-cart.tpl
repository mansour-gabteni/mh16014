...
<tbody>
                    ...
                    {if isset($cannotModify) AND $cannotModify == 1}
                        ...
                        <div id="cart_quantity_button" class="cart_quantity_button" style="float:left">
                            <a rel="nofollow" class="cart_quantity_up"
                               ...
                               href="{$link->getPageLink('cart', true, NULL, "add&amp;id_product={$product.id_product|intval}&amp;ipa={$product.id_product_attribute|intval}&amp;id_address_delivery={$product.id_address_delivery}&amp;id_customization={$id_customization}&amp;token={$token_cart}")}"
                               title="{l s='Add' mod='onepagecheckout'}"><img src="{$img_dir}icon/quantity_up.gif" alt="{l s='Add' mod='onepagecheckout'}"
                            ...
                            {if $product.minimal_quantity < ($customization.quantity -$quantityDisplayed) OR $product.minimal_quantity <= 1}
                                <a rel="nofollow" class="cart_quantity_down"
                                   ...
                                   href="{$link->getPageLink('cart', true, NULL, "add&amp;id_product={$product.id_product|intval}&amp;ipa={$product.id_product_attribute|intval}&amp;id_address_delivery={$product.id_address_delivery}&amp;id_customization={$id_customization}&amp;op=down&amp;token={$token_cart}")}"
                                   title="{l s='Subtract' mod='onepagecheckout'}">
                                    <img src="{$img_dir}icon/quantity_down.gif" alt="{l s='Subtract' mod='onepagecheckout'}" width="14"
                                         height="9"/>
                                </a>
                                {else}
                    ...
                    {if isset($cannotModify) AND $cannotModify == 1}
                        ...
                        <div>
                            <a rel="nofollow" class="cart_quantity_delete"
                               ...
                               href="{$link->getPageLink('cart', true, NULL, "delete&amp;id_product={$product.id_product|intval}&amp;ipa={$product.id_product_attribute|intval}&amp;id_customization={$id_customization}&amp;id_address_delivery={$product.id_address_delivery}&amp;token={$token_cart}")}"><i class=" fa fa-trash-o"></i></a>
                        </div>
                    {/if}
                </td>
            </tr>
    ...
    <tbody>
        ...
        <tr class="cart_discount {if $discount@last}last_item{elseif $discount@first}first_item{else}item{/if}"
            ...
            <td class="cart_discount_name" colspan="{$colspan2}">{$discount.name}</td>
            ...
            <td class="cart_discount_price">
                {if strlen($discount.code)}<a
                    ...
                    class="cart_quantity_delete" title="{l s='Delete' mod='onepagecheckout'}"><i class=" fa fa-trash-o"></i></a>{/if}<br />
                            ...
                            {foreach $displayVouchers as $voucher}
                                {if $voucher.code != ''}
                                    <span onclick="$('#discount_name').val('{$voucher.code}');return false;"
                                          class="voucher_name_opc">{$voucher.code}</span> - {/if}{$voucher.name}<br />
                            {/foreach}
                        </div>
                    {/if}
                </fieldset>
...
{if $use_taxes}
    {if $priceDisplay}
        <tr class="cart_total_products summary-line">
            <td colspan="{$colspan}">{if $display_tax_label}{l s='Total products (tax excl.):' mod='onepagecheckout'}{else}{l s='Total products:' mod='onepagecheckout'}{/if}</td>
            <td class="price" id="total_product">{displayPrice price=$total_products}</td>
        </tr>
    {else}
        <tr class="cart_total_products summary-line">
            <td colspan="{$colspan}">{if $display_tax_label}{l s='Total products (tax incl.):' mod='onepagecheckout'}{else}{l s='Total products:' mod='onepagecheckout'}{/if}</td>
            <td class="price" id="total_product">{displayPrice price=$total_products_wt}</td>
        </tr>
    {/if}
{else}
    <tr class="cart_total_products summary-line">
        <td colspan="{$colspan}">{l s='Total products:' mod='onepagecheckout'}</td>
        <td class="price" id="total_product">{displayPrice price=$total_products}</td>
    </tr>
{/if}
...
{if $total_shipping_tax_exc <= 0 && !isset($virtualCart)}
    <tr class="cart_total_delivery summary-line">
        <td colspan="{$colspan}">{l s='Shipping:' mod='onepagecheckout'}</td>
    ...
    {if $use_taxes}
        {if $priceDisplay}
            <tr class="cart_total_delivery summary-line" {if $total_shipping_tax_exc <= 0} style="display:none;"{/if}>
                <td colspan="{$colspan}">{if $display_tax_label}{l s='Total shipping (tax excl.):' mod='onepagecheckout'}{else}{l s='Total shipping:' mod='onepagecheckout'}{/if}</td>
        ...
        {else}
            <tr class="cart_total_delivery summary-line"{if $total_shipping <= 0} style="display:none;"{/if}>
                <td colspan="{$colspan}">{if $display_tax_label}{l s='Total shipping (tax incl.):' mod='onepagecheckout'}{else}{l s='Total shipping:' mod='onepagecheckout'}{/if}</td>
                <td class="price" id="total_shipping">{displayPrice price=$total_shipping}</td>
            </tr>
        {/if}
    {else}
        <tr class="cart_total_delivery summary-line"{if $total_shipping_tax_exc <= 0} style="display:none;"{/if}>
            <td colspan="{$colspan}">{l s='Total shipping:' mod='onepagecheckout'}</td>
            <td class="price" id="total_shipping">{displayPrice price=$total_shipping_tax_exc}</td>
        </tr>
    {/if}
{/if}
{if $use_taxes}
    <tr class="cart_tax_exc_price summary-line">
        <td colspan="{$colspan}">{l s='Total (tax excl.):' mod='onepagecheckout'}</td>
    ...
    <tr class="cart_total_tax summary-line">
        <td colspan="{$colspan}">{l s='Total tax:' mod='onepagecheckout'}</td>
        <td class="price" id="total_tax">{displayPrice price=$total_tax}</td>
    </tr>
{/if}
<tr class="cart_final_price summary-line">
    <td colspan="{$colspan}">{l s='Total:' mod='onepagecheckout'}</td>
