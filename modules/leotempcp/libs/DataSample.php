...
class Datasample
{
	...
	public function importDataByTable($content, $tableName)
	{
		...
		foreach ($content as $contentFields)
		{
			...
			foreach ($contentFields as $kF => $vF)
			{
				...
				else
				{
					if (isset($vF->attributes()->type))
					{
						...
						{
							$moduleId = Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue('SELECT id_module FROM `'._DB_PREFIX_.'module'.'` WHERE name=\''.$vF.'\'');
							if (!$moduleId)
							{
								$isCorrect = 0;
								break;
						...
						{
							$hookId = Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue('SELECT id_hook FROM `'._DB_PREFIX_.'hook'.'` WHERE name=\''.$vF.'\'');
			...
			{
				$sql = "INSERT IGNORE INTO `"._DB_PREFIX_."hook_module` (`id_module` ,`id_shop` ,`id_hook` ,`position`) VALUES (".$moduleId.",".$this->_id_shop.",".$hookId.",1)";
				Db::getInstance()->execute($sql);
			}
		}
	}
	...
	{
		$tableName = _DB_PREFIX_.$tableName;
		if (count(Db::getInstance()->executeS('SHOW TABLES LIKE \''.$tableName.'\'')))
		{
			$where = ' WHERE `'.$searchField.'`='.$searchValue;
			Db::getInstance()->execute("DELETE FROM `".$tableName."` ".$where);
		}
	}
	...
	public function deleteDataOfModByShop($objectMName, $searchField, $searchValue, $moduleName)
	{
		...
		{
			$schema = Db::getInstance()->executeS('SHOW CREATE TABLE `'.$tableName.'`');
		...
		if ($searchField != "")
			$where = ' WHERE `'.$searchField.'`='.$searchValue;
		$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('SELECT * FROM `'.$tableName.'`'.$where);
...
<dataSample>';
					...
					if (isset($moduleXml->table_prefix))
					{
						...
						{
							$listTable = Db::getInstance()->executeS("SHOW TABLES LIKE  '%".$pfl."%'");
	...
	public function buildStructNoneObject($field, $searchField, $searchValue, $stringField)
	{
		...
		$where = ' WHERE `'.$searchField.'`='.$searchValue;
		$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('SELECT * FROM `'._DB_PREFIX_.$field->table_name.'`'.$where);
	...
	public function buildStruct($field, $searchField, $searchValue, $moduleName, $isChild = 0, $result = array())
	{
		...
		if (!$isChild)
		{
			...
			if (isset($field->treeField) && $field->treeField)
			{
				...
				if ($tableShop != '')
				{
					$getIDs = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('SELECT `'.$defined['primary'].'` FROM '.$tableShop.$where);
					$arrayExport = array();
					...
					if ($arrayExport)
					{
						...
						$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('SELECT m.`'.$defined['primary'].'`, m.`'.$treeField.'` FROM `'.$tableName.'` as m'.$where);
					}
				}
				else
				{
					$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('SELECT DISTINCT m.`'.$defined['primary'].'`, m.`'.$treeField.'` FROM `'.$tableName.'` '.$tableShop.$where);
				}
				$children = array();
				foreach ($result as $v)
				{
	...
	public function createTableSample($module, $tablePrefix, $installFolder = "")
	{
		...
		{
			$listTmp = Db::getInstance()->executeS("SHOW TABLES LIKE  '%".$tablePrefix."%'");
			if ($preList)
	...
	public static function correctImgLink()
	{
		...
		foreach ($data as $row)
		{
			...
			$sql = 'UPDATE `'._DB_PREFIX_.'leowidgets` SET `params` = \''.base64_encode(Tools::jsonEncode($params)).'\' WHERE `id_leowidgets` = '.$row["id_leowidgets"];
			Db::getInstance()->execute($sql);
		}
	}
