		...
		<div class="pos">{l s='UnHook Modules'  mod='leotempcp'} </div>
		 {foreach from=$modules item=module name=leotempcp}
			<div class="module-pos" id="module-{$module->id}">
				<a class="editmod" href="{$moduleEditURL}&module_name={$module->name}&configure={$module->name}" data-mod="{$module->name}"></a>
	...
	<div class="leotheme-layout">
		<div id="leo-header">
			...
			<div class="module-pos" id="module-{$module['instance']->id}" data-position="{$module['id_hook']}">
				<a class="editmod" href="{$moduleEditURL}&module_name={$module['name']}&configure={$module['name']}" data-mod="{$module['name']}"></a><div class="edithook"></div>
...
$(".editmod").fancybox({
 	...
 	afterLoad:function(   ){
 		if( $('body',$('.fancybox-iframe').contents()).find("#main").length  ){  
 			...
 			$('body',$('.fancybox-iframe').contents()).find("#psException").html('<div class="alert error">{$noModuleConfig}</div>');
 		}
 	}
});
</script>
