{*
	...
	*}
	...
	<div  class="iqitmegamenu-wrapper col-xs-12 cbp-hor-width-{$menu_settings.hor_width}  clearfix">
		<div id="iqitmegamenu-horizontal{if isset($menu_settings_v) && ($menu_settings_v.ver_position == 6)}-sidebar{/if}" class="iqitmegamenu {if $menu_settings.hor_s_transparent && $menu_settings.hor_sticky} cbp-sticky-transparent{/if}" role="navigation">
			<div class="container">
	...
	{if $tab.url_type == 2}<a role="button" class="cbp-empty-mlink">{else}<a href="{$tab.url}" {if $tab.new_window}target="_blank"{/if}>{/if}
								...
								{if !$tab.icon_type && !empty($tab.icon)} <img src="{$tab.icon}" alt="{$tab.title}" class="cbp-mainlink-iicon" />{/if}{if !$tab.active_label}{$tab.title}{/if}{if $tab.submenu_type} <i class="icon-angle-down cbp-submenu-aindicator"></i>{/if}</span>
			...
			<div id="iqitmegamenu-mobile">
					...
					{if !$mobile_menu_style}<div id="cbp-spmenu-overlay" class="cbp-spmenu-overlay"></div>{/if}
				</div> 
		</div>
	</div>
