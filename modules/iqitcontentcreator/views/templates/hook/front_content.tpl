{*
	...
	*}
	...
	{if $node.type==1}
	<div class="row iqitcontent_row iqitcontent-element {if $node.depth==0} first_rows{/if} iqitcontent-element-id-{$node.elementId} {if isset($node.row_s.prlx) && $node.row_s.prlx}parallax-row{/if}">
	{if isset($node.row_s.bgw) && $node.row_s.bgw}<div class="iqit-fullwidth clearfix">{/if} 
	
	{elseif $node.type==2}
		<div  class="{if $node.width_p==13}hidden-xs{else}col-xs-{$node.width_p}{/if} {if $node.width_t==13}hidden-sm{else}col-sm-{$node.width_t}{/if} {if $node.width_d==13}hidden-md hidden-lg{else}col-md-{$node.width_d}{/if} iqitcontent-column iqitcontent-element iqitcontent-element-id-{$node.elementId} {if $node.contentType==0}empty-column{/if}" >
