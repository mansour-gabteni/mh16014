			...
			{if $node.type==2}
				...
				{elseif $node.contentType==2}
					...
					{if isset($node.content.products)}
						...
						{if $node.content.view==0}
							{include file="$tpl_dir./product-list.tpl" productimg=$node.content.productsimg products=$node.content.products generatorGrid="col-xs-{$node.content.line_xs} col-ms-{$node.content.line_ms} col-sm-{$node.content.line_sm} col-md-{$node.content.line_md} col-lg-{$node.content.line_lg}"}
						{elseif $node.content.view==1}
							{include file="$tpl_dir./product-slider.tpl" productimg=$node.content.productsimg ar=$node.content.ar ap=$node.content.ap  dt=$node.content.dt colnb=$node.content.colnb products=$node.content.products iqitGenerator=1 line_xs=$node.content.line_xs line_ms=$node.content.line_ms line_sm=$node.content.line_sm line_md=$node.content.line_md line_lg=$node.content.line_lg}
						{elseif $node.content.view==2}
							{include file="$tpl_dir./product-list-small.tpl" productimg=$node.content.productsimg products=$node.content.products generatorGrid="col-xs-{$node.content.line_xs} col-ms-{$node.content.line_ms} col-sm-{$node.content.line_sm} col-md-{$node.content.line_md} col-lg-{$node.content.line_lg}"}
						{else}
							{include file="$tpl_dir./product-list-small.tpl" productimg=$node.content.productsimg ar=$node.content.ar ap=$node.content.ap  dt=$node.content.dt colnb=$node.content.colnb products=$node.content.products  iqitGenerator=1 line_xs=$node.content.line_xs line_ms=$node.content.line_ms line_sm=$node.content.line_sm line_md=$node.content.line_md line_lg=$node.content.line_lg}
						{/if}
				...
				{elseif $node.contentType==6}
					...
					{if isset($node.content.source)}
						{if isset($node.content.href)}<a href="{$node.content.href}" {if isset($node.content.window) && $node.content.window == 1}target="_blank"{/if}>{/if}
				...
				{if $node.content.view}<div class="iqitcarousel-wrapper">{/if}
					...
					{assign var="iterator" value=1}
						{foreach from=$node.content.ids item=manufacturer name=manufacturerSlider}
								{assign var="myfile" value="img/m/{$manufacturer|escape:'htmlall':'UTF-8'}-mf_image.jpg"}
									{if file_exists($myfile)}
									{if $node.content.view}{if $iterator == 1}<div class="iqitcarousel-man">{/if}{/if}
							...
							{if $node.content.view}
								{if ($iterator%$node.content.colnb == 0) && !$smarty.foreach.manufacturerSlider.last}</div><div class="iqitcarousel-man">{/if}
