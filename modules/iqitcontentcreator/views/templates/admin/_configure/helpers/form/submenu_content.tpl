			...
			{if $node.type==4}
				...
				{if isset($node.children) && $node.children|@count > 0}
					{foreach from=$node.children item=child name=categoryTreeBranch}
						{if $child.type==3}
							<li class="iqitcontent-tab-li-id-{$child.elementId} {if $smarty.foreach.categoryTreeBranch.first} active{/if}"><a href="#iqitcontent-tab-id-{$child.elementId}"  data-toggle="tab">{foreach from=$languages item=language}
