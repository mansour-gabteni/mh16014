...
class Iqitcontentcreator extends Module
{
	...
	protected function renderForm()
	{
		...
		if($content)
			$content_format = $this->buildSubmenuTree(json_decode($content, true), false);
	...
	protected function _postProcess()
	{
		...
		foreach ($this->defaults as $default => $value)
		{
			if($default == 'content'){
				...
				Configuration::updateValue($this->config_name.'_'.$default, $default);
		}
		...
		{	
			if(isset($element['row_s']))
			{
				if(isset($element['row_s']['bgw']) && $element['row_s']['bgw'])
					$css .= '#iqitcontentcreator .iqitcontent-element-id-'.$element['elementId'].' .iqit-fullwidth{
					...
					$css .= ' #iqitcontentcreator .iqitcontent-element-id-'.$element['elementId'].'{
						...
						'.(isset($element['row_s']['br_top_st']) && $element['row_s']['br_top_st'] != '' ? 'border-top-style: '.$element['row_s']['br_top_st'].';' : '').'
						'.(isset($element['row_s']['br_top_wh']) && $element['row_s']['br_top_wh'] != '' ? 'border-top-width: '.$element['row_s']['br_top_wh'].'px;' : '').'
						...
						'.(isset($element['row_s']['br_bottom_st']) && $element['row_s']['br_bottom_st'] != '' ? 'border-bottom-style: '.$element['row_s']['br_bottom_st'].';' : '').'
						'.(isset($element['row_s']['br_bottom_wh']) && $element['row_s']['br_bottom_wh'] != '' ? 'border-bottom-width: '.$element['row_s']['br_bottom_wh'].'px;' : '').'
	...
	public function generateCss()
	{
		...
		if (Shop::getContext() == Shop::CONTEXT_GROUP)
			$my_file = $this->local_path.'css/iqitcreator_g_'.(int)$this->context->shop->getContextShopGroupID().'.css';
	...
	{	
		$random = date('Ymd').'|'.round(rand(1, max(4, 1)));

		if (!$this->isCached('iqitcontentcreator.tpl', $this->getCacheId($this->getCacheId('iqitcontentcreator|'.$random))))
		{
		...
		return $this->display(__FILE__, 'iqitcontentcreator.tpl', $this->getCacheId($this->getCacheId('iqitcontentcreator|'.$random)));
	...
	public function getProducts($ids)
	{
			...
			if (Group::isFeatureActive())
				...
				$selected_products = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
					SELECT DISTINCT p.id_product, pl.name, pl.link_rewrite, p.reference, i.id_image, product_shop.show_price, product_shop.`id_category_default`, 
    ...
    public function clearCreatorCache()
    {
         $this->_clearCache('*');
    }
