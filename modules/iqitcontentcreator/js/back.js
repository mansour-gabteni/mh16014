...
var p_auto_settingsf =  {
	...
	else
	{
		...
		$('.product-autocomplete').autocomplete('ajax_products_list.php', p_auto_settings).result(function(event, data, formatted) {
			if (data == null)
				return false;
...
$( '.grid_creator' ).on( 'click', '.menu-element .add-tab-action', function() {
	...
	$parentElement.find('.nav-tabs').first().append( '<li class="iqitcontent-tab-li-id-' + (elementId) + ' active"><a href="#iqitcontent-tab-id-' + (elementId) + '" data-toggle="tab">Tab ' + (elementId) + '</a></li>' );
...
$('.menu-element-id-' + elementId + ' .spectrumcolor').spectrum({
	...
	if (typeof iqit_frontcreator != 'undefined' && iqit_frontcreator)
	{
		...
		$('.menu-element-id-' + elementId + ' .product-autocomplete').autocomplete('ajax_products_list.php', p_auto_settings).result(function(event, data, formatted) {
			if (data == null)
				return false;
			var productId = data.id;
			var productName = data.name;
...
$( '.grid_creator' ).on( 'click', '.menu-element .add-tabs-action', function() 
{
	...
	if (typeof iqit_frontcreator != 'undefined' && iqit_frontcreator)
		$parentElement.append( '<div data-element-type="4" data-depth="' + depth + '"  data-width-p="12" data-width-t="12" data-width-d="12" data-element-id="' + (++elementId) + '" class="col-xs-12 col-sm-12 col-md-12 iqitcontent-column iqitcontent-element menu_column menu_tabs menu-element menu-element-id-' + (elementId) + ' clearfix">' + $( '#buttons-sample' ).html() + $( '#column-content-sample' ).html() + '<ul class="nav nav-tabs"></ul><div class="tab-content"></div></div>' );
	else
		$parentElement.append( '<div data-element-type="4" data-depth="' + depth + '"  data-width-p="12" data-width-t="12" data-width-d="12" data-element-id="' + (++elementId) + '" class="creator-col-xs-12 creator-col-sm-12 creator-col-md-12 menu_column menu_tabs menu-element menu-element-id-' + (elementId) + '">' + $( '#buttons-sample' ).html() + $( '#column-content-sample' ).html() + ' <ul class="nav nav-tabs"></ul><div class="tab-content"></div></div>' );
