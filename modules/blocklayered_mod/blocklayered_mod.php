...
class BlockLayered_mod extends Module
{
	private $products;
	...
	{
		if (parent::install() && $this->registerHook('header') && $this->registerHook('leftColumn') && $this->registerHook('aboveProductList')
		...
		&& $this->registerHook('postProcessAttribute') && $this->registerHook('afterDeleteAttribute') && $this->registerHook('afterSaveAttribute') && $this->registerHook('leftColumn'))
		{
			...
			Configuration::updateValue('PS_LAYERED_STICKY', 0);
			Configuration::updateValue('PS_LAYERED_ABOVE', 0);
	...
	public function uninstall()
	{
		...
		Configuration::deleteByName('PS_LAYERED_STICKY');
		Configuration::deleteByName('PS_LAYERED_ABOVE');
	...
	public function hookLeftColumn($params)
	{
		...
		if(Configuration::get('PS_LAYERED_ABOVE'))
			return;
	...
	public function hookAboveProductList($params)
	{	
		if(!Configuration::get('PS_LAYERED_ABOVE'))
			return;

		return '<section id="center-layered-nav" class="clearfix"><div class="container">'.$this->generateFiltersBlock($this->getSelectedFilters()).'</div></section>';
	}
	...
	public function hookHeader($params)
	{
		...
		$this->context->controller->addJS(($this->_path).'blocklayered_mod.js');
		...
		if (Configuration::get('PS_LAYERED_STICKY') && Configuration::get('PS_LAYERED_ABOVE'))
			$this->context->controller->addJS(($this->_path).'blocklayered_mod_stick.js');

		if (version_compare(_PS_VERSION_, '1.6.0', '>=') === true)
			$this->context->controller->addCSS(($this->_path).'blocklayered_mod.css', 'all');
		else
			$this->context->controller->addCSS(($this->_path).'blocklayered_mod-15.css', 'all');
	...
	public function hookFooter($params)
	{
		...
		if (Dispatcher::getInstance()->getController() == 'category')
			$this->context->controller->addJS($this->_path.'blocklayered_mod-footer.js');
	}
	...
	private static function indexPrices($cursor = null, $full = false, $ajax = false, $smart = false)
	{
		...
		{
			$token = substr(Tools::encrypt('blocklayered_mod/index'), 0, 10);
			if (Tools::usingSecureMode())
				$domain = Tools::getShopDomainSsl(true);
			else
				$domain = Tools::getShopDomain(true);

			if (!Tools::file_get_contents($domain.__PS_BASE_URI__.'modules/blocklayered_mod/blocklayered_mod-price-indexer.php?token='.$token.'&cursor='.(int)$cursor.'&full='.(int)$full))
	...
	public function getContent()
	{
		...
		{
			Configuration::updateValue('PS_LAYERED_STICKY', (int)Tools::getValue('ps_layered_sticky'));
			Configuration::updateValue('PS_LAYERED_ABOVE', (int)Tools::getValue('ps_layered_above'));
		...
		{
			$this->context->controller->addJS($this->_path.'js/blocklayered_mod_admin.js');
		...
		if (version_compare(_PS_VERSION_, '1.6.0', '>=') === true)
			$this->context->controller->addCSS($this->_path.'css/blocklayered_mod_admin_1.6.css');
		else
			$this->context->controller->addCSS($this->_path.'css/blocklayered_mod_admin.css');

		if (Tools::getValue('add_new_filters_template'))
		{
			$this->context->smarty->assign(array(
				'current_url' => $this->context->link->getAdminLink('AdminModules').'&configure=blocklayered_mod&tab_module=front_office_features&module_name=blocklayered_mod',
				'uri' => $this->getPathUri(),
				'id_layered_filter' => 0,
		...
		else
		{
			$this->context->smarty->assign(array(
				...
				'price_indexer_url' => $module_url.'blocklayered_mod-price-indexer.php'.'?token='.substr(Tools::encrypt('blocklayered_mod/index'), 0, 10),
				'full_price_indexer_url' => $module_url.'blocklayered_mod-price-indexer.php'.'?token='.substr(Tools::encrypt('blocklayered_mod/index'), 0, 10).'&full=1',
				'attribute_indexer_url' => $module_url.'blocklayered_mod-attribute-indexer.php'.'?token='.substr(Tools::encrypt('blocklayered_mod/index'), 0, 10),
				'url_indexer_url' => $module_url.'blocklayered_mod-url-indexer.php'.'?token='.substr(Tools::encrypt('blocklayered_mod/index'), 0, 10).'&truncate=1',
				...
				'sticky' => Configuration::get('PS_LAYERED_STICKY'),
				'above' => Configuration::get('PS_LAYERED_ABOVE'),
	...
	public function getFilterBlock($selected_filters = array())
	{
		...
		foreach ($filters as $filter)
		{
			...
			foreach ($filters as $filter_tmp)
			{
				...
				if (method_exists('BlockLayered_mod', $method_name) &&
	...
	public function generateFiltersBlock($selected_filters)
	{
		...
		if ($filter_block = $this->getFilterBlock($selected_filters))
		{
			...
			$smarty->assign(array(
				...
				'blocklayered_modSliderName' => $translate,
				'col_img_dir' => _PS_COL_IMG_DIR_
			));
			return $this->display(__FILE__, 'blocklayered_mod.tpl');
		}
		else
			return false;
	}
