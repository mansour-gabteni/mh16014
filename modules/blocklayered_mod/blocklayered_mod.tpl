...
<h4 class="title_block layeredSortBy">{l s='Sort by:' mod='blocklayered_mod'}</h4>
		<form action="#" id="layered_form">
			<div>
				{if isset($selected_filters) && $n_filters > 0}
				<div id="enabled_filters">
						<div class="title_block layered_enabledsubtitle"><span class="layered_subtitle" style="float: none;">
						{l s='Enabled filters:' mod='blocklayered_mod'}
					</span></div>
					<ul>
									...
									{if $filter.type == $filter_type && isset($filter.values)}
										{if isset($filter.slider)}
											{if $smarty.foreach.f_values.first}
												<li>
													<a href="#" data-rel="layered_{$filter.type}_slider" title="{l s='Cancel' mod='blocklayered_mod'}"><i class="icon-remove"></i></a>
													{if $filter.format == 1}
														{l s='%1$s: %2$s - %3$s'|sprintf:$filter.name:{displayPrice price=$filter.values[0]}:{displayPrice price=$filter.values[1]}|escape:'html':'UTF-8' mod='blocklayered_mod'}
													{else}
														{l s='%1$s: %2$s %4$s - %3$s %4$s'|sprintf:$filter.name:$filter.values[0]:$filter.values[1]:$filter.unit|escape:'html':'UTF-8' mod='blocklayered_mod'}
													{/if}
										...
										{else}
											{foreach from=$filter.values key=id_value item=value}
												{if $id_value == $filter_key && !is_numeric($filter_value) && ($filter.type eq 'id_attribute_group' || $filter.type eq 'id_feature') || $id_value == $filter_value && $filter.type neq 'id_attribute_group' && $filter.type neq 'id_feature'}
													<li>
														<a href="#" data-rel="layered_{$filter.type_lite}_{$id_value}" title="{l s='Cancel' mod='blocklayered_mod'}"><i class="icon-remove"></i></a>
														{l s='%1$s: %2$s' mod='blocklayered_mod' sprintf=[$filter.name, $value.name]}
													</li>
												{/if}
											{/foreach}
										{/if}
				...
				{foreach from=$filters item=filter}
					{if isset($filter.values)}
							{if isset($filter.slider)}
							<div class="layered_{$filter.type} layered_filter_center" style="display: none;">
						{else}
							<div class="layered_filter layered_filter_center">
						{/if}
                        <div class="layered_subtitle_heading title_block">
                            <div><span class="layered_subtitle ">{$filter.name|escape:'html':'UTF-8'} </span></div>
						</div>
						<ul id="ul_layered_{$filter.type}_{$filter.id_key}" class="col-lg-12 toogle_content layered_filter_ul{if isset($filter.is_color_group) && $filter.is_color_group} color-group{/if}">
							{if !isset($filter.slider)}
								{if $filter.filter_type == 0}
									{foreach from=$filter.values key=id_value item=value name=fe}
										...
										<li class="nomargin {if $smarty.foreach.fe.index >= $filter.filter_show_limit}hiddable{/if} col-xs-12">
											...
											{/if}
											<label for="layered_{$filter.type_lite}_{$id_value}"{if !$value.nbr} class="disabled"{else}{if isset($filter.is_color_group) && $filter.is_color_group} name="layered_{$filter.type_lite}_{$id_value}" class="layered_color" data-rel="{$id_value}_{$filter.id_key}"{else} class="layered_nocolor"{/if}{/if}>
												{if !$value.nbr}
								...
								{else}
									...
									{else}
										<select class="select form-control" {if $filter.filter_show_limit > 1}multiple="multiple" size="{$filter.filter_show_limit}"{/if}>
											<option value="">{l s='No filters' mod='blocklayered_mod'}</option>
											{foreach from=$filter.values key=id_value item=value}
											{if $value.nbr || !$hide_0_values}
							...
							{else}
								{if $filter.filter_type == 0}
									<label for="{$filter.type}">
										{l s='Range:' mod='blocklayered_mod'}
									</label> 
								...
								{else}
									...
									{foreach from=$filter.list_of_values  item=values}
										<li class="nomargin {if $filter.values[1] == $values[1] && $filter.values[0] == $values[0]}layered_list_selected{/if} layered_list" onclick="$('#layered_{$filter.type}_range_min').val({$values[0]});$('#layered_{$filter.type}_range_max').val({$values[1]});reloadContent();">
											- {l s='From' mod='blocklayered_mod'} {$values[0]} {$filter.unit} {l s='to' mod='blocklayered_mod'} {$values[1]} {$filter.unit}
										</li>
									{/foreach}
							...
							{if count($filter.values) > $filter.filter_show_limit && $filter.filter_show_limit > 0 && $filter.filter_type != 2}
								<span class="hide-action more">{l s='Show more' mod='blocklayered_mod'}</span>
								<span class="hide-action less">{l s='Show less' mod='blocklayered_mod'}</span>
							{/if}
						</ul>
					</div>
					{/if}
...
{strip}
	...
	{/if}
	{addJsDef blocklayered_modSliderName=$blocklayered_modSliderName}
	{addJsDef blocklayeredSliderName=$blocklayered_modSliderName}
