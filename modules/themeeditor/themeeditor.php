		...
		$fields_form_main = array(
			...
			'form' => array(
				...
				'input' => array(
						...
						array(
						'type' => 'select',
						'label' => $this->l('Variant'),
						'upper_separator' => true,
						'row_title' => $this->l('Cart and blockuserinfo'),
						'name' => 'cart_style',
						'options' => array(
							'query' => array(
								array(
								'id_option' => 1,
								'name' => $this->l('Big icon/narrow - variant-2')
								),
							array(
								'id_option' => 0,
								'name' => $this->l('Default(small icon/wide - variant-1)')
								)
							),                           
    						'id' => 'id_option',                           
    						'name' => 'name'
  ...
  .product_list.list .ajax_block_product:hover + li .product-container, .product_list_small .ajax_block_product:hover .product-container {
    border-color: transparent !important; }
		...
		if(Configuration::get($this->configName.'_product_colors')==1)
			$css .= '.ajax_block_product .hovered .color-list-container{display: block !important;}';
		...
		ul.product_list .availability span.out-of-stock, .slick_carousel .availability span.out-of-stock, #availability_statut #availability_value.warning_inline,
...
#search_block_top  .form-control::-webkit-input-placeholder {
  color: '.Configuration::get($this->configName.'_header_input_text').' !important; }
		...
		if(Configuration::get($this->configName.'_header_absolute'))
			...
			if(Configuration::get($this->configName.'_header_style') == 0)
			$css .= '
		...
		elseif($content_bg_type==0)
		{
			...
			$css .= '#columns .content-inner, body.content_only, #layer_cart{color: '.Configuration::get($this->configName.'_content_txt_color').' !important; }  
  ...
  padding: 0px 20px;
	  ...
	  #columns .content-inner .nav-tabs  li:first-child  {border-left: none; padding-left: 0px;}
	...
	public function hookHeader($params) {
		...
		if (isset($_SERVER['HTTP_USER_AGENT']) && 
			...
			$theme_settings = array(
				...
				'grid_size_ms' => Configuration::get($this->configName.'_grid_size_ms') - $inc,
				'grid_size_xs' => Configuration::get($this->configName.'_grid_size_xs') - $inc,
