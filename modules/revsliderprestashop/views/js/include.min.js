function fix_colums(e) {
    ...
    if ($("#view").val() > 0) {
        if ($("#view").val() == 1) $("ul.grid li, ul.grid figure").css("width", "100%");
        else {
        ...
        $("#help").hide()
    } else Modernizr.touch && $("#help").show()
}
...
function apply(e, t) {
    ...
    o = o.toLowerCase();
    var u = "",
        a = new Array("ogg", "mp3", "wav"),
        f = new Array("mp4", "ogg", "webm");
    if ($.inArray(o, ext_img) > -1) u = '<img src="' + i + r + e + '" alt="' + s + '" />';
    else if ($.inArray(o, f) > -1) u = '<video controls source src="' + i + r + e + '" type="video/' + o + '">' + s + "</video>";
    else if ($.inArray(o, a) > -1) {
        o == "mp3" && (o = "mpeg");
        u = '<audio controls src="' + i + r + e + '" type="audio/' + o + '">' + s + "</audio>"
    } else u = '<a href="' + i + r + e + '" title="' + s + '">' + s + "</a>";
...
function apply_none(e, t) {
    ...
    if (n.html() != "" && n.html() !== undefined) {
        ...
        n.hasClass("audio") ? $(".body-preview").css("height", "80px") : $(".body-preview").css("height", "345px");
        $.ajax({
...
function close_window() {
    $("#popup").val() == 1 ? window.close() : typeof parent.jQuery != "undefined" && parent.jQuery ? parent.jQuery.fancybox.close() : parent.$.fancybox.close()
}
...
function apply_file_rename(e, t) {
    ...
    var o = e.find("a.preview"),
        r = o.attr("data-url");
    typeof r != "undefined" && r && o.attr("data-url", r.replace(encodeURIComponent(i), encodeURIComponent(t + "." + s)));
    e.parent().attr("data-name", t + "." + s);
    e.attr("data-name", t + "." + s);
    e.find(".name_download").val(t + "." + s);
    var u = e.find("a.rename-file"),
        a = e.find("a.delete-file"),
        f = u.attr("data-path"),
        l = u.attr("data-thumb"),
        c = f.replace(i, t + "." + s),
        h = l.replace(i, t + "." + s);
...
function apply_folder_rename(e, t) {
    ...
    var r = e.find(".folder-link"),
        i = r.attr("href"),
        s = $("#fldr_value").val(),
        o = i.replace("fldr=" + s + encodeURIComponent(n), "fldr=" + s + encodeURIComponent(t));
    r.each(function() {
        $(this).attr("href", o)
    });
    var u = e.find("a.delete-folder"),
        a = e.find("a.rename-folder"),
        f = a.attr("data-path"),
        l = a.attr("data-thumb"),
        c = f.lastIndexOf("/"),
        h = f.substr(0, c + 1) + t;
...
function replaceDiacritics(e) {
    var e, t = [/[\300-\306]/g, /[\340-\346]/g, /[\310-\313]/g, /[\350-\353]/g, /[\314-\317]/g, /[\354-\357]/g, /[\322-\330]/g, /[\362-\370]/g, /[\331-\334]/g, /[\371-\374]/g, /[\321]/g, /[\361]/g, /[\307]/g, /[\347]/g],
        n = ["A", "a", "E", "e", "I", "i", "O", "o", "U", "u", "N", "n", "C", "c"];
    for (var r = 0; r < t.length; r++) e = e.replace(t[r], n[r]);
    return e
}
...
function sortUnorderedList(e, t, n) {
    ...
    $.each(r, function(e) {
        ...
        if ($.isNumeric(r)) {
            r = parseFloat(r);
            while (typeof s[r] != "undefined" && s[r]) r = parseFloat(parseFloat(r) + parseFloat(.001))
        } else r = r + "a" + t.find("h4 a").attr("data-file");
        s[r] = t.html();
        o.push(r)
    });
    $.each(i, function(e) {
        ...
        if ($.isNumeric(r)) {
            r = parseFloat(r);
            while (typeof u[r] != "undefined" && u[r]) r = parseFloat(parseFloat(r) + parseFloat(.001))
        } else r = r + "a" + t.find("h4 a").attr("data-file");
        u[r] = t.html();
        a.push(r)
    });
    $.isNumeric(o[0]) ? o.sort(function(e, t) {
        return parseFloat(e) - parseFloat(t)
    }) : o.sort();
    $.isNumeric(a[0]) ? a.sort(function(e, t) {
        return parseFloat(e) - parseFloat(t)
    }) : a.sort();
    if (t) {
        o.reverse();
...
var version = "9.3.3",
    active_contextmenu = !0;
loading_bar && (/MSIE (\d+\.\d+);/.test(navigator.userAgent) ? $(document).ready(function() {
    $("body").queryLoader2({
...
$(document).ready(function() {
    if (active_contextmenu) {
        $.contextMenu({
            ...
            build: function(e) {
                    ...
                    callback: function(t, n) {
                        switch (t) {
                            ...
                            case "unzip":
                                ...
                                $.ajax({
                                    type: "POST",
                                    url: ajax_calls_url+"?action=extract",
                                    data: {
                                        path: r
                                    }
                                }).done(function(e) {
                                    e != "" ? bootbox.alert(e) : window.location.href = $("#refresh").attr("href") + "&" + (new Date).getTime()
                                });
                                break;
                            case "edit_img":
                                var i = e.attr("data-name"),
                                    s = $("#base_url_true").val() + $("#cur_dir").val() + i;
                ...
                (e.find(".img-precontainer-mini .filetype").hasClass("png") || e.find(".img-precontainer-mini .filetype").hasClass("jpg") || e.find(".img-precontainer-mini .filetype").hasClass("jpeg")) && image_editor && (t.items.edit_img = {
                ...
                if (e.find(".img-precontainer-mini .filetype").hasClass("zip") || e.find(".img-precontainer-mini .filetype").hasClass("tar") || e.find(".img-precontainer-mini .filetype").hasClass("gz")) t.items.unzip = {
                    name: $("#lang_extract").val(),
                    icon: "extract",
                    disabled: !1
                };
                !e.hasClass("directory") && $("#duplicate").val() == 1 && (t.items.duplicate = {
        ...
        $(document).on("contextmenu", function(e) {
            if (!$(e.target).is("figure")) return !1
        })
    }
    ...
    $("ul.grid").on("click", ".modalAV", function(e) {
        ...
        _this.hasClass("audio") ? $(".body-preview").css("height", "80px") : $(".body-preview").css("height", "345px");
        $.ajax({
    ...
    $("input[name=radio-sort]").on("click", function() {
        ...
        if (e == "ff-item-type-all") $(".grid li").show(300);
    ...
    if (parseInt($("#file_number").val()) > parseInt($("#file_number_limit_js").val())) var t = !1;
    else var t = !0;
    $("#filter-input").on("keyup", function() {
        ...
        $(this).val(n);
        e(function() {
            t && $("ul.grid li").each(function() {
                var e = $(this);
                n != "" && e.attr("data-name").toString().toLowerCase().indexOf(n.toLowerCase()) == -1 ? e.hide(100) : e.show(100)
            })
        }, 300)
    }).keypress(function(e) {
        e.which == 13 && $("#filter").trigger("click")
    });
    ...
    $(".sorter").on("click", function() {
        ...
        if (t) {
            $.ajax({
                url: ajax_calls_url+"?action=sort&sort_by=" + _this.attr("data-sort") + "&descending=" + n
            }).done(function(e) {});
            sortUnorderedList("ul.grid", n, "." + _this.attr("data-sort"));
            $(" a.sorter").removeClass("descending").removeClass("ascending");
            n ? $(".sort-" + _this.attr("data-sort")).addClass("descending") : $(".sort-" + _this.attr("data-sort")).addClass("ascending")
        } else window.location.href = $("#current_url").val() + "&sort_by=" + _this.attr("data-sort") + "&descending=" + n
    });
    ...
    $("ul.grid").on("click", ".preview", function() {
        ...
        e.hasClass("disabled") == 0 && show_animation();
        return !0
    });
    ...
    $("ul.grid").on("click", ".rename-file", function() {
        ...
        bootbox.prompt($("#rename").val(), $("#cancel").val(), $("#ok").val(), function(n) {
            if (n !== null) {
                n = fix_filename(n);
                n != r && execute_action("rename_file", e.attr("data-path"), e.attr("data-thumb"), n, t, "apply_file_rename")
            }
        }, r)
    });
    $("ul.grid").on("click", ".rename-folder", function() {
        ...
        bootbox.prompt($("#rename").val(), $("#cancel").val(), $("#ok").val(), function(n) {
            if (n !== null) {
                ...
                n != r && execute_action("rename_folder", e.attr("data-path"), e.attr("data-thumb"), n, t, "apply_folder_rename")
            }
        }, r)
    });
    ...
    $(".view-controller button").on("click", function() {
        ...
        e.find("i").addClass("icon-white");
        $.ajax({
            url: ajax_calls_url+"?action=view&type=" + e.attr("data-value")
        }).done(function(e) {
            e != "" && bootbox.alert(e)
        });
        typeof $("ul.grid")[0] != "undefined" && $("ul.grid")[0] && ($("ul.grid")[0].className = $("ul.grid")[0].className.replace(/\blist-view.*?\b/g, ""));
        typeof $(".sorter-container")[0] != "undefined" && $(".sorter-container")[0] && ($(".sorter-container")[0].className = $(".sorter-container")[0].className.replace(/\blist-view.*?\b/g, ""));
        var t = e.attr("data-value");
        $("#view").val(t);
        $("ul.grid").addClass("list-view" + t);
        $(".sorter-container").addClass("list-view" + t);
        if (e.attr("data-value") >= 1) fix_colums(14);
        else {
    ...
    if (!Modernizr.touch) {
        ...
        $("figure").bind("mouseover", function() {
            $("#view").val() == 0 && $(this).find(".box:not(.no-effect)").animate({
                top: "-30px"
            }, {
                queue: !1,
                duration: 300
