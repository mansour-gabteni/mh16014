...
{block name="field"}
		...
		<div class="form-group">
			<label class="control-label col-lg-3 " for="title"> {l s='Title' mod='leomanagewidgets'} </label>
			<div class="col-lg-9">
        ...
        <div class="form-group">
			<label class="control-label col-lg-3 " for="link"> {l s='Link' mod='leomanagewidgets'} </label>
			<div class="col-lg-9">
				{foreach from=$languages item=language}
	...
	<div class="form-group">
				<label class="control-label col-lg-3 " for="description"> {l s='Description' mod='leomanagewidgets'} </label>
				<div class="col-lg-9">
					{foreach from=$languages item=language}
			...
			<div class="form-group">
				<label class="control-label col-lg-3 " for="title"> {l s='Image' mod='leomanagewidgets'} </label>
				<div class="col-lg-9">
						{foreach from=$languages item=language}
					...
					<a href='{$input.selectImg}&imgDir=slideImg' data-type='main' class='choose-img'>
						{l s='Choose Image' mod='leomanagewidgets'}
					</a>	
				</div>
			</div>	
			<div class="form-group">
				<label class="control-label col-lg-3 " for="title"> {l s='Thumb' mod='leomanagewidgets'} </label>
				<div class="col-lg-9">
						{foreach from=$languages item=language}
						...
						{/foreach}
                                                <a href='{$input.selectImg}&imgDir=slideThumb' data-type='thumb' class='choose-img'>
                                                        {l s='Choose Thumbnail' mod='leomanagewidgets'}
                                                </a>
				</div>
			</div>	
			<div class="form-group">
				<div  class="col-lg-9 col-lg-offset-3">
						<button class="btn btn-default addslide" name="addslide" type="button">
							...
							{l s='Add' mod='leomanagewidgets'}
						</button>
						<input class="btn-update btn btn-info updateslide" type="button" name="updateslide" value="{l s='Update' mod='leomanagewidgets'}">
				</div>	
			</div>	
			<hr>
			<div class="form-group">
					 ...
					 {foreach from=$items item=item}
						...
						<div id="contentSlides_{$item.id_slide}" class="leo-slide col-lg-12" style="border: 1px solid #ccc;margin-top: 5px;padding: 5px;">
							...
							<div class="col-lg-2">
								...
								<p>
									<a class="btn-edit btn btn-info" href="{$url}&id_slide={$item.id_slide}" {if $slide_edit && $slide_edit.id_slide && $slide_edit.id_slide == $item.id_slide}style="display:none"{/if}/>{l s='Edit' mod='leomanagewidgets'}</a>
									<input class="btn-delete btn btn-danger" type="button" id="{$item.id_slide}" value="{l s='Delete' mod='leomanagewidgets'}">
								</p>
								</div>
							</div>
		...
		{if $input.type == 'setting_tab'}
			...
			<hr>	
					<div class="form-group">
									<label class="control-label col-lg-3 " for="tabtitle"> {l s='Title' mod='leomanagewidgets'} </label>
									<div class="col-lg-9">
										{foreach from=$languages item=language}
										{assign var="t_title" value="title_`$language['iso_code']`"}
											{if $languages|count > 1}
							...
							<div class="form-group">
									<label class="control-label col-lg-3 " for="tabtype"> {l s='Type' mod='leomanagewidgets'} </label>
									<div class="col-lg-9">
										<div class="col-lg-9 ">
											<select id="tabtype" class=" fixed-width-xl" name="tabtype">
												<option value="all">{l s='--All--' mod='leomanagewidgets'}</option>
												<option value="special" {if  $tab_edit && $tab_edit.type == 'special'} selected="selected" {/if} >{l s='Special' mod='leomanagewidgets'}</option>
												<option value="bestseller" {if  $tab_edit && $tab_edit.type == 'bestseller'} selected="selected" {/if}>{l s='BestSeller' mod='leomanagewidgets'}</option>
												<option value="featured" {if  $tab_edit && $tab_edit.type == 'featured'} selected="selected" {/if}>{l s='Featured' mod='leomanagewidgets'}</option>
												<option value="new" {if  $tab_edit && $tab_edit.type == 'new'}	selected="selected" {/if}>{l s='New Arrials' mod='leomanagewidgets'}</option>
											</select>
										</div>	
									</div>
							</div>
							<div class="form-group">
									<label class="control-label col-lg-3 " for="icon"> {l s='Icon' mod='leomanagewidgets'} </label>
									<div class="col-lg-9">
										<div class="col-lg-9 ">
											<select id="icon" class=" fixed-width-xl" name="icon">
												<option value="none">{l s='--None--' mod='leomanagewidgets'}</option>
												{if $images}
					...
					<div class="form-group">
						<div  class="col-lg-9 col-lg-offset-3">
							...
							<input class="btn-update btn btn-info updatetabs" type="button" name="updatetabs" value="{l s='Update' mod='leomanagewidgets'}">
						</div>	
					</div>	
					<div class="form-group">
						<div class="col-lg-offset-3">
							<div  class="col-lg-9">
								<div class="col-lg-1">{l s='Id' mod='leomanagewidgets'}</div>
								<div class="col-lg-2">{l s='Title' mod='leomanagewidgets'}</div>
								<div class="col-lg-3">{l s='Type' mod='leomanagewidgets'}</div>
								<div class="col-lg-3" >{l s='Categories' mod='leomanagewidgets'}</div>
								<div class="col-lg-1" >{l s='Icon' mod='leomanagewidgets'}</div>
								<div class="col-lg-2 " style="text-align:right;">{l s='Action' mod='leomanagewidgets'}</div>
							</div>
						</div>	
					</div>
					<div class="form-group">
							 ...
							 {foreach from=$items item=item}
								<div id="contentTabs_{$item.id_tab}" class="leo-tab col-lg-9" style="border: 1px solid #ccc;margin-top: 5px;padding: 5px;">
									...
									<div class="col-lg-2">
										<div class="btn-group-action pull-right">
										<p>
												<a class="btn-edit btn btn-info" href="{$url}&id_tab={$item.id_tab}" {if $tab_edit && $tab_edit.id_tab && $tab_edit.id_tab == $item.id_tab}style="display:none"{/if}/>{l s='Edit' mod='leomanagewidgets'}</a>
											<input class="btn-delete btn btn-danger" type="button" id="{$item.id_tab}" value="{l s='Delete' mod='leomanagewidgets'}">
										</p>
										</div>
									</div>
