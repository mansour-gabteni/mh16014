...
class LeoWidgetNewsletter extends LeoWidgetBase
{
	...
	private function getNewsletter($setting)
	{
		...
		if ($this->error)
		{
			...
			$setting['nw_value'] = Tools::getIsset(Tools::getValue('email')) ? pSQL(Tools::getValue('email')) : false;
	...
	private function newsletterRegistration()
	{
		...
		else if (Tools::getValue('action') == '1')
		{
			...
			{
				if (!Db::getInstance()->execute('DELETE FROM '._DB_PREFIX_.'newsletter WHERE `email` = \''.pSQL(Tools::getValue('email')).'\' AND id_shop = '.$this->context->shop->id))
			...
			{
				if (!Db::getInstance()->execute('UPDATE '._DB_PREFIX_.'customer SET `newsletter` = 0 WHERE `email` = \''.pSQL(Tools::getValue('email')).'\' AND id_shop = '.$this->context->shop->id))
					return $this->error = $this->l('An error occurred while attempting to unsubscribe.');
				return $this->valid = $this->l('Unsubscription successful');
			}
		}
