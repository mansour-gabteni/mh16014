...
class CombinationCore extends ObjectModel
{
	...
	public function setImages($ids_image)
	{
		...
		WHERE ag.is_color_group=1 and pac.`id_product_attribute` = '.(int)$this->id;
		
		if (!$row = Db::getInstance()->executeS($sql)){
			$id_product_attributes[] = $this->id;
		} else {	
			$id_attribute = $row[0]['id_attribute'];
			...
			inner join `'._DB_PREFIX_.'attribute_group` ag on a.id_attribute_group = ag.id_attribute_group
			inner join `'._DB_PREFIX_.'product_attribute` pa on pa.id_product_attribute = pac.id_product_attribute 
			WHERE ag.is_color_group=1 and pa.id_product = '.(int) $this->id_product.' 
			and a.id_attribute = '.(int) $id_attribute;
			
			if ($row = Db::getInstance()->executeS($sql))
				foreach ($row as $id_product_attribute)
		...
		foreach ($id_product_attributes as $id_product_attribute) {
			...
			if (is_array($ids_image) && count($ids_image))
			{
				...
				foreach ($ids_image as $value)
					$sql_values[] = '('.(int)$id_product_attribute.', '.(int)$value.')';
