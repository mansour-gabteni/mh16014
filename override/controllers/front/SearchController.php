	...
	public function initContent()
		...
		if ($this->instant_search && !is_array($query))
            ...
            if (method_exists('Product', 'getProductsImgs'))
            {
            	...
            	{
            		if (isset($search['result'][$i]['id_product'])) $image_array[$search['result'][$i]['id_product']] = Product::getProductsImgs($search['result'][$i]['id_product']);
            	}
            ...
            $this->context->smarty->assign(array(
                'products' => $search['result'],                 'search_products' => $search['result'],
                'nbProducts' => $search['total'],
		...
		elseif (($tag = urldecode(Tools::getValue('tag'))) && !is_array($tag))
			...
			if (method_exists('Product', 'getProductsImgs'))
				...
				for ($i = 0; $i < $nbProducts; $i++)
					{
					if (isset($result[$i]['id_product'])) $image_array[$result[$i]['id_product']] = Product::getProductsImgs($result[$i]['id_product']);
					}
			...
			$this->context->smarty->assign(array(
				'search_tag' => $tag,
				'products' => $result, 				'search_products' => $result,
