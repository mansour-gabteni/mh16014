...
<form action="{$request_uri|escape:'html':'UTF-8'}" method="post" class="std" id="form_forgotpassword">
	<fieldset>
		...
		<p class="submit">
            <button type="submit" class="btn btn-outline button button-medium"><span>{l s='Retrieve Password'}</span></button>
		</p>
	</fieldset>
</form>
{/if}
...
<ul class="clearfix footer_links">
	<li class="pull-left"><a class="btn btn-outline button button-small" href="{$link->getPageLink('authentication')|escape:'html':'UTF-8'}" title="{l s='Back to Login'}" rel="nofollow"><span>{l s='Back to Login'}</span></a></li>
</ul>
