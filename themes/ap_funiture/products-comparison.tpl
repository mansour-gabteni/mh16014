								...
								<div class="button-container">
									{if (!$product->hasAttributes() OR (isset($add_prod_display) AND ($add_prod_display == 1))) AND $product->minimal_quantity == 1 AND $product->customizable != 2 AND !$PS_CATALOG_MODE}
										{if ($product->quantity > 0 OR $product->allow_oosp)}
											<a
												class="button ajax_add_to_cart_button btn btn-outline"
												data-id-product="{$product->id}"
										...
										{else}
											<span class="ajax_add_to_cart_button button btn btn-outline disabled">
												<span>{l s='Out of stock'}</span>
											</span>
										{/if}
									{/if}
									<a 
									class="button lnk_view btn btn-outline" 
									href="{$product->getLink()|escape:'html':'UTF-8'}" 
									title="{l s='View'}">
										<span>{l s='View'}</span>
									</a>
						...
						<div class="comparison_product_infos">
							<p class="comparison_availability_statut">
								{if !(($product->quantity <= 0 && !$product->available_later) OR ($product->quantity != 0 && !$product->available_now) OR !$product->available_for_order OR $PS_CATALOG_MODE)}
									...
									<span class="availability_value label label-success"{if $product->quantity <= 0} class="warning-inline"{/if}>
...
<ul class="footer_link">
	<li>
		<a class="button lnk_view btn btn-outline" href="{$base_dir}">
