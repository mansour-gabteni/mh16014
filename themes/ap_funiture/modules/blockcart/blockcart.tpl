...
<div class="blockcart_top clearfix{if $PS_CATALOG_MODE} header_user_catalog{/if} pull-right">
{/if}
		...
		<div class="heading">
			<a href="{$link->getPageLink($order_process, true)|escape:'html':'UTF-8'}" title="{l s='View my shopping cart' mod='blockcart'}" rel="nofollow">
				<div class="cart-inner media-body">
					...
					<span class="ajax_cart_product_txt_s" {if $cart_qties <= 1}style="display:none"{/if}>{l s='Items' mod='blockcart'}</span>
					<span class="ajax_cart_product_txt" {if $cart_qties > 1}style="display:none"{/if}>{l s='Item' mod='blockcart'}</span>
					<span class="ajax_cart_no_product{if $cart_qties > 0} unvisible{/if}">{l s='0 Item' mod='blockcart'}</span>					
					</div>
					<span class="title">{l s='Shopping bag' mod='blockcart'} <i class="fa fa-angle-down"></i></span> 
				...
				<div class="block_content">
					...
					<div class="cart_block_list{if isset($blockcart_top) && !$blockcart_top}{if isset($colapseExpandStatus) && $colapseExpandStatus eq 'expanded' || !$ajax_allowed || !isset($colapseExpandStatus)} expanded{else} collapsed unvisible{/if}{/if}">
											...
											{foreach from=$customizedDatas.$productId.$productAttributeId[$product.id_address_delivery] key='id_customization' item='customization' name='customizations'}
												<li name="customization">
													<div data-id="deleteCustomizableProduct_{$id_customization|intval}_{$product.id_product|intval}_{$product.id_product_attribute|intval}_{$product.id_address_delivery|intval}" class="deleteCustomizableProduct">
														<a class="ajax_cart_block_remove_link" href="{$link->getPageLink('cart', true, NULL, "delete=1&id_product={$product.id_product|intval}&ipa={$product.id_product_attribute|intval}&id_customization={$id_customization|intval}&token={$static_token}")|escape:'html':'UTF-8'}" rel="nofollow"><i class="fa fa-remove"></i></a>
													</div>
						...
						<p class="cart-buttons clearfix">
							<a id="button_order_cart" class="btn btn-outline button-medium button button-small pull-right" href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}" title="{l s='Check out' mod='blockcart'}" rel="nofollow">
								<span>
		...
		<div class="clearfix">
			...
			<div class="layer_cart_cart col-xs-12 col-md-6">
				...
				<div class="button-container">	
					...
					<a class="btn btn-outline-inverse button pull-right" href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}" title="{l s='Proceed to checkout' mod='blockcart'}" rel="nofollow">
						<span>
