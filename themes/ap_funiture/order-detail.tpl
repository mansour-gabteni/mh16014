	...
	<div class="row">
		<div class="col-xs-12 col-sm-6"{if $order->isVirtual()} style="display:none;"{/if}>
			<ul class="address alternate_item box">
				<li><h3>{l s='Delivery address'} ({$address_delivery->alias})</h3></li>
		...
		<div class="col-xs-12 col-sm-6">
			<ul class="address item {if $order->isVirtual()}full_width{/if} box">
				<li><h3>{l s='Invoice address'} ({$address_invoice->alias})</h3></li>
					...
					<tr class="item">
						...
						<input class="order_qte_input form-control grey"  name="order_qte_input[{$smarty.foreach.products.index}]" type="text" size="2" value="{$product.customizationQuantityTotal|intval}" />
							<div class="clearfix return_quantity_buttons">
								<a href="#" class="return_quantity_down btn btn-outline button-minus"><span><i class="fa fa-minus"></i></span></a>
								<a href="#" class="return_quantity_up btn btn-outline button-plus"><span><i class="fa fa-plus"></i></span></a>
							</div>                        
...
{if $return_allowed}
	<div id="returnOrderMessage">
		...
		<p class="form-group">
			<button type="submit" name="submitReturnMerchandise" class="btn btn-outline button button-small"><span>{l s='Make an RMA slip'}</span></button>
...
{if $carriers|count > 0 && isset($carriers.0.carrier_name) && $carriers.0.carrier_name}
	<table class="table table-responsive table-bordered footab">
		<thead>
			<tr>
				<th class="first_item">{l s='Date'}</th>
