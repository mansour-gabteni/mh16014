	...
	<div class="row">
		<div class="col-xs-12 col-sm-6"{if $order->isVirtual()} style="display:none;"{/if}>
			<ul class="address alternate_item box">
				<li><h3 class="page-subheading"><span>{l s='Delivery address'} ({$address_delivery->alias})</span></h3></li>
		...
		<div class="col-xs-12 col-sm-6">
			<ul class="address item {if $order->isVirtual()}full_width{/if} box">
				<li><h3 class="page-subheading"><span>{l s='Invoice address'} ({$address_invoice->alias})</span></h3></li>
...
{if $return_allowed}
	<div id="returnOrderMessage">
		<h3 class="page-subheading bottom-indent"><span>{l s='Merchandise return'}</span></h3>
...
{if !$is_guest}
	{if count($messages)}
	<h3 class="page-subheading"><span>{l s='Messages'}</span></h3>
	...
	<form action="{$link->getPageLink('order-detail', true)|escape:'html':'UTF-8'}" method="post" class="std" id="sendOrderMessage">
		<h3 class="page-subheading bottom-indent"><span>{l s='Add a message'}</span></h3>
