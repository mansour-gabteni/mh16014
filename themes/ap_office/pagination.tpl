...
{if isset($p) AND $p}
	...
	{/if}
	<!-- Pagination -->
	<div id="pagination{if isset($paginationId)}_{$paginationId}{/if}" class="pagination clearfix pull-left">
	    {if $nb_products > $products_per_page && $start!=$stop}
			<form class="showall pull-left" action="{if !is_array($requestNb)}{$requestNb}{else}{$requestNb.requestUrl}{/if}" method="get">
				<div>
