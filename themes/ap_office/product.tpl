				...
				<div id="short_description_block">
					...
					{if $product->description}
						<p class="buttons_bottom_block">
							<a href="javascript:{ldelim}{rdelim}" class="button btn btn-default">
								{l s='More details'}
							</a>
						</p>
					{/if}
...
{if ($display_qties == 1 && !$PS_CATALOG_MODE && $PS_STOCK_MANAGEMENT && $product->available_for_order)}			
			...
			<p>
				<span class="availability_date_label">{l s='Availability date:'}</span>
				<span class="availability_value2">{l s='Availability duaring'}</span>
			</p>			
							...
							<div class="price">
								<p id="old_price"{if (!$product->specificPrice || !$product->specificPrice.reduction) && $group_reduction == 0} class="hidden"{/if}>
									{if $priceDisplay >= 0 && $priceDisplay <= 2}
										...
										<span id="old_price_display" class="old-price product-price">{if $productPriceWithoutReduction > $productPrice}{convertPrice price=$productPriceWithoutReduction}{/if}</span>
